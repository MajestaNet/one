...
5. Read [docs/api-families.md](docs/api-families.md) (overview), the matching [`docs/api/`](docs/api/) family page, and [ADR-004](docs/adr/004-three-api-families.md) when the change touches HTTP or family ownership (BP-010 mitigated). Customer endpoint catalogs are `docs/api/{client,metadata,deploy,ops,auth}.md` — do not paste the overview five times. Public objects: [docs/objects.md](docs/objects.md) (not `GET /describe`).
6. Read [backlog/README.md](backlog/README.md) for foreseeable architecture remainders (`BP-*.md`). Confirmed SI-campaign **defects** are GitHub issues (`[campaign G-…]` / `[campaign S-…]`) in [docs/customer-rollout-gap-log.md](docs/customer-rollout-gap-log.md) — fix those with `Fixes #<issue>`; do **not** open a new BP from a campaign beat. Treat each BP `Area:` as the preferred code scope (resolve via the module map).
7. Prefer solutions that reduce backlog risk (especially High severity) when the request is open-ended.
8. When you close or materially de-risk a backlog item, update its status in the item file and the table in `backlog/README.md`. When you close a campaign GitHub issue, mark that beat `closed` in [docs/customer-rollout-gap-log.md](docs/customer-rollout-gap-log.md) and link the PR (`Fixes #<issue>`).
...
| Control IDE (Electron / React / Vitest) — optional client; **lockstep refactor OK** when it cleans the install ([BP-065](backlog/BP-065-ide-backend-coupling.md)); demo-client honesty ([BP-066](backlog/BP-066-ide-demo-client-fidelity.md)); brand restyle ([BP-068](backlog/BP-068-ide-brand-visual.md) mitigated); no new Electron-only product chrome | [agent-control-ide.md](docs/architecture/agent-control-ide.md) · [ide-demo-client-uplift-build-plan.md](docs/architecture/ide-demo-client-uplift-build-plan.md) · [ide-brand-visual-build-plan.md](docs/architecture/ide-brand-visual-build-plan.md) | `control-ide` |
...
| SI customer-install simulation (dev/test/prod, packages, automations at scale, IDE UX) — **execute** the campaign; do not start it from a docs-only task | [customer-install-simulation-playbook.md](docs/architecture/customer-install-simulation-playbook.md) · [customer-install-simulation-test-run.md](docs/customer-install-simulation-test-run.md) | executor (then domain agent on `Fixes #N`) |
