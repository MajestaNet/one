...
- Reintroducing Node/OpenAPI stubs (ADR-005). A later **generated** OpenAPI artifact for `one.majesta.net` is Phase 5 of [public-docs-site-build-plan.md](./public-docs-site-build-plan.md) — not those stubs.
...
- [ ] Customer-facing route changes are covered by the public docs writer path: optional same-PR edit of the mapped `/api/...` page, otherwise the merge-event docs agent ([agent-public-docs.md](./agent-public-docs.md) · [public-docs-site-build-plan.md](./public-docs-site-build-plan.md); GitHub `docs/` until `tools/one-docs` exists)
