...
| Managed package automations (seed on enable, Metadata `active` toggle, description, IDE consume) | [package-automations-build-plan.md](./package-automations-build-plan.md) + api-families + data (+ control-ide for Phase 6) | `api-families` then `db-backend-perf` then `worker-jobs` then `deploy-ops`; `control-ide` after HTTP | Go: `packages`, `seed`, `metadata`, `httpapi`, `dataengine`, `automation`, `deploy`, `migrations/`; IDE: `tools/control-ide/**` only in Phase 6 |
...
| Control IDE UI, Electron, Vite, Vitest, panels (optional client; lockstep OK for [BP-065](../../backlog/BP-065-ide-backend-coupling.md); demo honesty [BP-066](../../backlog/BP-066-ide-demo-client-fidelity.md); brand restyle [BP-068](../../backlog/BP-068-ide-brand-visual.md) mitigated) | [agent-control-ide.md](./agent-control-ide.md) · [ide-demo-client-uplift-build-plan.md](./ide-demo-client-uplift-build-plan.md) · [ide-brand-visual-build-plan.md](./ide-brand-visual-build-plan.md) | `control-ide` | `tools/control-ide/**` only (plus mapped Go when the task is install-coupling cleanup or a BP-066 API-gap handoff) |
